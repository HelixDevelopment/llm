...
// EX-28 guard for the IMAGE-GENERATION boot path: the storage axis of the
...
//	             ran on (measured on the video sibling: 73x different fit
//	             answers between /dev/shm and the volume's filesystem on one
//	             host).
...
func TestWeightsVolumeMatchesComposeFile(t *testing.T) {
	...
	b, err := os.ReadFile("compose.imagegen.yml")
	if err != nil {
		t.Fatalf("read compose.imagegen.yml: %v", err)
	}
	if !strings.Contains(string(b), weightsVolume) {
		t.Fatalf("compose.imagegen.yml does not reference %q — resolver and compose file have drifted apart",
			weightsVolume)
	}
	// The mount is the runtime's HF cache; the compose file must say so.
	if !strings.Contains(string(b), "/models/hf") {
		t.Fatal("compose.imagegen.yml no longer mounts the weights volume at /models/hf — " +
